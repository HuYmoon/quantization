...
def data_procession(data1, data2, data3, n):
    """
    数据预处理
    :param data1: 股票闭市价格(pre+data+later)
    :param data2: 净利润(data, 财务报表, 季度数据，计算因子值的分子）, 数据组包含的股票个数少于data1
     :param data3: 总市值（计算因子值的分母）
    :param n: 调仓期
    :return:
    """
    # 根据调仓期匹配交易日
    ...
    data3 = data3.loc[data1.index[1:-1], :]
    print(data1.shape)
    print(data2.shape)
    print(data3.shape)
    ...
    # EP, 股票名称为净利润和总价值的交集
    same_col = list(set(data2.columns).intersection(data3.columns))
    factor_values = {}
    for stock_name in same_col:
        # 数据框转list,list(df.values),如果直接取list(df),返回的结果是列名
        factor_values[stock_name] = list((data2.loc[:, [stock_name]].div(data3.loc[:, [stock_name]])).values)

    # 字典转化为数据框
    factor_values = DataFrame(factor_values, index=data2.index)
...
def main():
    ...
    data1 = pd.read_csv('2010-2018的股票闭市价格.csv', index_col=0)   # 计算收益率
    data2 = pd.read_csv('2010-2018的净利润+日期.csv', index_col=0)   # 因子值的分子
    data3 = pd.read_csv('2010-2018年总市值.csv', index_col=0)   # 因子值的分母
    data3 = data3*100000000   # 总市值单位为亿，净利润单位为元
    print(data1.shape)
    print(data2.shape)
    print(data3.shape)
    print('数据读入完毕')
    ...
    factor_values, net = data_procession(data1, data2, data3, 5)
    # print(factor_values)
    print('数据预处理完成')
